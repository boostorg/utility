...
*/
#ifndef BOOST_UTILITY_OSTREAM_JOINER_HPP
#define BOOST_UTILITY_OSTREAM_JOINER_HPP
...
public:
    ...
    ostream_joiner(ostream_type& output, const Delim& delim)
        : output_(boost::addressof(output))
        , delim_(delim)
        , first_(true) { }
...
inline ostream_joiner<typename decay<Delim>::type, Char, Traits>
make_ostream_joiner(std::basic_ostream<Char, Traits>& output, Delim&& delim)
{
    return ostream_joiner<typename decay<Delim>::type, Char, Traits>(output,
